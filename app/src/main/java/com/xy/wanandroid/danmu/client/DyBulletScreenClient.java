package com.xy.wanandroid.danmu.client;
...
public class DyBulletScreenClient {
    ...
    public interface HandleMsgListener {
        void handleMessage(String nickname, String txt);
    }
...
//            }
            ...
            LogUtil.e("getServerMsg" + dataStr);
            MsgView msgView = new MsgView(dataStr);
            //分析该包的数据类型，以及根据需要进行业务操作
            parseServerMsg(msgView.getMessageList());

            /**
             * type@=chatmsg/rid@=319721/ct@=1/uid@=164932284/nn@=一缕胭脂未冷/txt@=我三科课代表骄傲了吗
             * /cid@=c791a236ff534e816607050000000000/ic@=avanew@Sface@S201803@S06@S19@S67eaaccc529e5ab693a58374c9756690/level@=1/sahf@=0/cst@=1532514003044/bnn@=/bl@=0/brid@=0/hc@=/el@=/lk@=/��
             */
        } catch (Exception e) {
            e.printStackTrace();
        }
